...
public static class BloodcraftAPI
{
    ...
    public static (int FirstUnarmed, int SecondUnarmed, int ClassSpell)? GetPlayerSpells(ulong steamId)
    {
    ...
    public static void SetPlayerSpells(ulong steamId, int firstUnarmed, int secondUnarmed, int classSpell)
    {
    ...
    [
        BloodType.Worker, BloodType.Warrior, BloodType.Scholar, BloodType.Rogue,
        BloodType.Mutant, BloodType.Draculin, BloodType.Immortal, BloodType.Creature,
        BloodType.Brute, BloodType.Corruption
    ];
    ...
    {
        if (steamId.TryGetPlayerPrestiges(out var prestiges) && prestiges.TryGetValue(prestigeType, out int level))
            return level;
    ...
    public static Dictionary<string, List<FamiliarData>> GetBoxes(ulong steamId)
    {
        ...
        foreach (var box in unlocksData.FamiliarUnlocks)
        {
            ...
            foreach (int prefabHash in box.Value)
            {
                ...
                familiars.Add(new FamiliarData
                {
    ...
    public static void SetFamiliarExperience(ulong steamId, int familiarPrefabHash, int level, float xp)
    {
        var data = LoadFamiliarExperienceData(steamId);
    ...
    public static bool DeleteBox(ulong steamId, string boxName)
    {
        ...
        if (!data.FamiliarUnlocks.TryGetValue(boxName, out var familiarSet) || familiarSet.Count != 0)
            return false;
    ...
    public static bool MoveFamiliarToBox(ulong steamId, int familiarPrefabHash, string destinationBox)
    {
        var data = LoadFamiliarUnlocksData(steamId);

        if (!data.FamiliarUnlocks.TryGetValue(destinationBox, out var destSet) || destSet.Count >= BOX_SIZE)
            return false;
    ...
    public static bool RemoveFamiliarToOverflow(ulong steamId, string boxName, int familiarPrefabHash)
    {
        var data = LoadFamiliarUnlocksData(steamId);
    ...
    public static bool AddFamiliarToBox(ulong steamId, int familiarPrefabHash, string boxName)
    {
        ...
        if (!data.FamiliarUnlocks.TryGetValue(boxName, out var familiarSet) || familiarSet.Count >= BOX_SIZE)
            return false;
    ...
    public static QuestData? GetQuest(ulong steamId, QuestType questType)
    {
        ...
        if (steamId.TryGetPlayerQuests(out var quests) && quests.TryGetValue(internalType, out var quest))
        {
